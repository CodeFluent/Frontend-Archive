...
<div style="font-size: 16px; font-weight: bold; margin-right: 60px; text-align: right;"><a href="?layout=subuser">Back</a></div>
...
?>            
             <tr>
                ...
                <td colspan="2">
                	<input type="checkbox" name="eemail" value="user" <?php if($var[0]=='user'){?>checked="checked"<?php } ?> />&nbsp;<label><strong>Users</strong></label>&nbsp;
                    <input type="checkbox" name="form" value="template" <?php if($var[1]=='template'){?>checked="checked"<?php } ?> />&nbsp;<label><strong>Email Templates</strong></label>&nbsp;
                    <input type="checkbox" name="list" value="ltemplate" <?php if($var[2]=='ltemplate'){?>checked="checked"<?php } ?> />&nbsp;<label><strong>Lead Templates</strong></label>&nbsp;<br />
                    <input type="checkbox" name="video" value="video" <?php if($var[3]=='video'){?>checked="checked"<?php } ?> />&nbsp;<label><strong>Video</strong></label>&nbsp;
                    <input type="checkbox" name="photo" value="photo" <?php if($var[4]=='photo'){?>checked="checked"<?php } ?> />&nbsp;<label><strong>Photo</strong></label>&nbsp;
                    <input type="checkbox" name="drip" value="pricing" <?php if($var[5]=='pricing'){?>checked="checked"<?php } ?> />&nbsp;<label><strong>Pricing</strong></label>&nbsp;
                    <input type="checkbox" name="lead" value="page" <?php if($var[6]=='page'){?>checked="checked"<?php } ?> />&nbsp;<label><strong>Pages</strong></label>
                </td>
            </tr>
...
if($_POST['save']!='')
{
	$privileges = $_POST['eemail']."|".$_POST['form']."|".$_POST['list']."|".$_POST['video']."|".$_POST['photo']."|".$_POST['drip']."|".$_POST['lead'];			
	...
	{
		$res = $setting_obj->INSERT_SUB_USER($_SESSION['adminid'],$_POST['password'],$_POST['name'],$_POST['lname'],$_POST['email'],$_POST['contact'],$_POST['address'],$_POST['city'],$_POST['state'],$_POST['country'],$_POST['zip_code'],$filename,$privileges);
	...
	header("Location: index.php?layout=add-user&suid=".$suid);
}
?>    
